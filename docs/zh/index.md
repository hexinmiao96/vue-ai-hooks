...
features:
  - title: 五个组合式函数，一套心智模型
    details: useChat、useCompletion、useEmbedding、useGeneration 和 useObject 覆盖对话、文本、向量、自定义生成任务和结构化 JSON。可插拔 Provider 让你切换模型时不需要重写应用。
  ...
  - title: 一致的状态模型
    details: status、isLoading、error 和 clearError 在聊天、补全、向量、自定义生成与结构化对象输出中使用同一套生命周期语义。
  - title: TypeScript 优先
