...
| React 聊天最小接入     | 在小 React 应用里使用 `vue-ai-hooks/react`         | [React hooks](/zh/reference/react)             | 发送一次 prompt，调用 `stop()`，检查错误态 |
...
React 入口是迁移面，不改变项目 Vue-first 的定位，但 React 消费者可以复用同一套 provider 和 proxy 契约：
