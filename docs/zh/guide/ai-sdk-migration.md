...
这些 refs 会展示默认值、单次调用选项、proxy `api`、credentials、body、headers 和
retry attempt 合并后的最终 provider/proxy 请求。默认 proxy transport 下，
`prepareSendMessagesRequest`、`prepareStep` 和 `prepareReconnectToStreamRequest`
也会收到同一组 `api` 和 `credentials` 字段。
