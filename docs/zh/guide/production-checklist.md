...
- 需要可复制的 Nuxt/Nitro、Next.js、Hono 或 Fetch 投影路由时，从
  [Agent 路由模板](/zh/guide/agent-route-templates) 开始。
...
12. 运行 `pnpm agent-route-templates:check`，确认可复制 agent 路由模板通过 Nuxt/Nitro、
    Next.js、Hono、Fetch 和 LangGraph resume 形态的 executable fixture smoke。
13. 刷新一个服务端存储的 thread，确认 messages 恢复后仍保留 `Date` 值。
14. 重新生成一条 assistant 消息，从同一条 user 消息创建 branch，刷新后确认两个分支都能恢复，
    且没有重复写入相同 `runId`。
15. 发送过期 branch `revision`，确认返回 `branch_revision_conflict`；在一个 run streaming
    时启动第二次 regenerate，确认返回 `run_in_progress` 或安全恢复同一个 `runId`。
16. 分别审批通过和拒绝一个特权工具请求，确认重复提交相同 `runId` 不会执行两次，
    且过期审批 `revision` 返回 `approval_revision_conflict`。
17. 确认日志里有 trace id，但没有 Provider API key。
