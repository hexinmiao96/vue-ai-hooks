...
7. 刷新一个服务端存储的 thread，确认 messages 恢复后仍保留 `Date` 值。
8. 重新生成一条 assistant 消息，从同一条 user 消息创建 branch，刷新后确认两个分支都能恢复，
   且没有重复写入相同 `runId`。
9. 分别审批通过和拒绝一个特权工具请求，确认重复提交相同 `runId` 不会执行两次。
10. 确认日志里有 trace id，但没有 Provider API key。
