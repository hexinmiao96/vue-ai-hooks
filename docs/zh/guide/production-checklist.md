...
- 上游错误要归一化成用户安全的提示。
- 入口路由收到非法 `/api/chat` 载荷（缺失 runId/threadId/messages 数组）时，返回
  `HTTP 400` 和 `{"error":"Agent route failed"}`，并确保审计日志不含敏感信息。
...
- 需要可复制的 Nuxt/Nitro、Next.js、Hono、Express、Fastify、Cloudflare 或 Fetch
  投影路由时，从 [Agent 路由模板](/zh/guide/agent-route-templates) 开始。
...
- 过期审批 `revision` 要在工具执行前返回 `approval_revision_conflict`。
...
`pnpm production:readiness` 和 `pnpm production:readiness:local` 使用同一个
Node runner，因此发布候选门禁只有一份执行定义。它包含完整的 `pnpm check`
门禁、`pnpm release:cadence` 和 `pnpm release:status`。
...
6. 发送一次格式错误的 `/api/chat` 请求（缺失 `runId`、`threadId` 或 messages 数组），
   确认响应为 `400` 且 body 为 `{"error":"Agent route failed"}`，日志不含敏感信息。
7. 捕获一份 `inspectRequestTrace()` snapshot，确认没有 secret。
8. 运行 `pnpm image:check`，确认图片生成/编辑请求会保留源图、mask 和 trace metadata。
9. 运行 `pnpm react-video:check`，确认 React 视频 demo 保留确定性 storyboard、proxy 和
   trace 契约。
10. 运行 `pnpm threaded-chat:check`，确认本地 thread index 和每个 thread 的 message
    storage 可以独立恢复。
11. 运行 `pnpm agent-run:check`，确认 interrupt/resume、同 run 重放和检查 trace 行为保持稳定。
12. 运行 `pnpm demo-ux:check`，确认 completion/object demo 的输出摘要、trace 可见性和 curl 复制链路。
13. 运行 `pnpm agent-route-templates:check`，确认可复制 agent 路由模板通过 Nuxt/Nitro、
    Next.js、Hono、Express、Fastify、Cloudflare、Fetch 和 LangGraph resume 形态的
    executable fixture smoke。
14. 运行 `pnpm competitive-benchmark:check`，确认竞品对标文档、threaded chat、路由模板文档
    和 API 导出面是否一致。
15. 刷新一个服务端存储的 thread，确认 messages 恢复后仍保留 `Date` 值。
16. 重新生成一条 assistant 消息，从同一条 user 消息创建 branch，刷新后确认两个分支都能恢复，
    且没有重复写入相同 `runId`。
17. 发送过期 branch `revision`，确认返回 `branch_revision_conflict`；在一个 run streaming
    时启动第二次 regenerate，确认返回 `run_in_progress` 或安全恢复同一个 `runId`。
18. 分别审批通过和拒绝一个特权工具请求，确认重复提交相同 `runId` 不会执行两次，
    且过期审批 `revision` 返回 `approval_revision_conflict`。
19. 确认日志里有 trace id，但没有 Provider API key。
