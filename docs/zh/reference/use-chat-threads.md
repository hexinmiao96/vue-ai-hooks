...
`CreateChatThreadInput`、`UpdateChatThreadInput`、`ChatThreadsPersistenceErrorInfo`、
`ChatThreadsPersistenceErrorPhase`、`ChatThreadsPersistOptions`、`UseChatThreadsOptions`
和 `UseChatThreadsReturn`。
...
| `persistenceError`        | `ComputedRef<ChatThreadsPersistenceErrorInfo \| null>`                                  | 最近一次 thread 索引 storage 失败摘要。  |
...
| `clearPersistenceError()` | `() => void`                                                                            | 清空最近一次 thread 索引 storage 错误。  |

当 `persist.onLoadError`、`persist.onError` 或 `persist.onClearError` 触发时，
`persistenceError` 会记录安全摘要。摘要包含 `phase`、`key`、可选 `version`、
`message`、可选 `name` 和 `timestamp`；不包含 thread payload、消息正文、
provider 密钥或原始错误 cause。
