...
const sidebarNavigationLabel = computed(() =>
  isChineseDemoContext.value || lang.value.startsWith('zh-') ? '侧边栏导航' : 'Sidebar Navigation'
)
...
onMounted(() => {
  if (!props.open && previousBodyOverflow.value) {
    unlockBodyScroll()
  }
})
