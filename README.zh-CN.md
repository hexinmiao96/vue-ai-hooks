...
这是 **v0.10.1**，是一个可工作的基础版本，但还不是功能完备版本。核心能力已覆盖主要
composable、Provider/proxy 适配、工具流、持久化、重试、流数据、metadata、共享状态和
质量门禁。本版本进入生产级增强线，先补齐更完整的 inspection timeline、retry、provider trace、可复制
curl、任务型 demo guide 和生产检查清单。下一步重点是 Provider 配方和 agent-event adapter
配方。功能规划放在
