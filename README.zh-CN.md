...
这些组合式函数也会暴露 `lastRequest`、`lastResponse` 和 `clearTrace()`，方便在不手动同步 lifecycle callback 的情况下保留最近一次 Provider 请求/响应 trace。默认 proxy trace 会包含解析后的 proxy `api` 和浏览器 credentials 模式。
可以使用 `inspectRequestTrace()` 把这些 refs 组合成生产调试 snapshot，里面包含
timeline events、归一化重试记录、紧凑 provider trace，以及可选的脱敏 `curl` 命令，便于支持排查。支持面板应渲染 `inspect()` / `inspectRequestTrace()` 输出，而不是直接展示原始 trace refs。
...
这是 **v1.0.0-rc.1**，是面向稳定 1.0 采用面的 release candidate。核心能力已覆盖主要
