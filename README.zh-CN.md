...
- **可选 React 聊天、补全和结构化对象支持**：从 `vue-ai-hooks/react` 导入
  `useChat`、`useCompletion` 或 `useObject`，在 React 中复用同一套 Provider 和请求类型管理流式状态。
...
- **生产聊天工作流**：服务端代理路径、可恢复流、thread 上下文、请求准备钩子、自定义
  body、metadata 和请求追踪。
- **AI SDK 风格 UI 和 agent helper**：`sendMessage`、工具输出/审批别名、文件附件、结构化
...
后端代理环境变量配方见 [Proxy 配方](https://github.com/hexinmiao96/vue-ai-hooks/blob/main/docs/zh/guide/proxy-recipes.md)。生产上线前请使用 [生产检查清单](https://github.com/hexinmiao96/vue-ai-hooks/blob/main/docs/zh/guide/production-checklist.md)。
...
这是 **v0.13.0**，是一个可工作的基础版本，但还不是功能完备版本。核心能力已覆盖主要
composable、Provider/proxy 适配、工具流、持久化、重试、流数据、metadata、共享状态和
质量门禁。本版本新增轻量 `AgentEvent` adapter，可把自有 agent 的 progress、tool、
source、file、error 和 finish 事件归一成 `ChatChunk` 或 AI SDK UI message stream parts。
下一步重点是会话/thread 持久化和生产 agent 配方。功能规划放在
